...
// Step labels and their progress ranges
// Ranges are weighted: extraction is slow, OCR optional, AI is the longest
const LABELS = ['Extracting text', 'OCR Processing', 'Identifying entities', 'Analyzing sentiment', 'Generating summary']

// Progress ceiling per step — step i is "active" while progress is in [START[i], START[i+1])
// Final 5% (95–100) is reserved for actual API response arrival
const STEP_START = [0, 22, 38, 56, 74]   // when each step becomes active
const STEP_DONE  = [22, 38, 56, 74, 95]  // when each step becomes done (simulated cap)

// Per-step status messages shown in the info bar
const STEP_MESSAGES: Record<number, string[]> = {
  0: ['Parsing document structure...', 'Reading page content...', 'Extracting raw text...', 'Detecting layout...'],
  1: ['Running OCR on scanned content...', 'Enhancing image quality...', 'Recognizing characters...', 'Processing page regions...'],
  2: ['Detecting organizations and names...', 'Scanning for dates and amounts...', 'Identifying key entities...', 'Mapping entity relationships...'],
  3: ['Evaluating document tone...', 'Scoring sentiment signals...', 'Analyzing language patterns...', 'Classifying emotional context...'],
  4: ['Composing concise summary...', 'Synthesizing key findings...', 'Finalizing insights...', 'Preparing analysis output...'],
}
...
export default function Processing() {
  ...
  // Rotate step-specific messages every 1.8s
  useEffect(() => {
    if (!file) return
  ...
  // ── Progress controller ──────────────────────────────────
  // Key design:
  // - Simulated progress crawls slowly up to the ceiling of the current backend step
  // - When apiStep advances, the ceiling rises — progress can catch up
  // - Hard cap at 95% until actual API response arrives
  // - On apiDone, rush to 100%
  useEffect(() => {
    if (!file) return
    let cancelled = false
    ...
    const startDelay = setTimeout(() => {
      ...
      const id = setInterval(() => {
        ...
        setProgress(prev => {
          // API done — rush to 100 at a steady pace
          if (apiDone) {
            if (prev >= 100) { clearInterval(id); return 100 }
            return Math.min(100, prev + 2.5)
          }

          // Determine ceiling from backend step signal
          // apiStep -1 = no signal yet → ceiling is STEP_DONE[0] (22%)
          // apiStep 0..4 → ceiling is STEP_DONE[apiStep], max 95
          const stepCeiling = apiStep >= 0
            ? Math.min(95, STEP_DONE[Math.min(apiStep, 4)])
            : STEP_DONE[0]

          if (prev >= stepCeiling) return prev  // hold — wait for next backend event

          // Speed: fast early, slow as we approach ceiling
          const gap = stepCeiling - prev
          const speed = gap > 20 ? 0.9 : gap > 10 ? 0.5 : gap > 4 ? 0.25 : 0.1

          return Math.min(stepCeiling, prev + speed)
        })
      }, 80)
  ...
  // ── Derived state ─────────────────────────────────────────
  // Which step is currently active based on progress
  const activeStepIdx = useMemo(() => {
    for (let i = STEP_START.length - 1; i >= 0; i--) {
      if (progress >= STEP_START[i]) return i
    }
    return 0
  }, [progress])
  ...
  // Current message for the active step
  const currentMsg = useMemo(() => {
    const msgs = STEP_MESSAGES[activeStepIdx] || STEP_MESSAGES[0]
    return msgs[msgIdx % msgs.length]
  }, [activeStepIdx, msgIdx])

  const handleCancel = () => { cancelledRef.current = true; nav('/') }
  ...
  const statusText = progress >= 100 ? 'Analysis complete'
    : activeStepIdx === 4 ? 'Generating summary...'
    : activeStepIdx === 3 ? 'Analyzing sentiment...'
    : activeStepIdx === 2 ? 'Identifying entities...'
    : activeStepIdx === 1 ? 'Running OCR...'
    : 'Extracting text...'
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: '#07070f' }}>
      ...
      {/* ── NAVBAR ── */}
      <nav className="relative z-50 shrink-0" style={{ height: '64px', background: 'rgba(10,10,20,0.75)', borderBottom: '1px solid rgba(255,255,255,0.06)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
          <Logo size="md" linkTo="/" />
          <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2">
            <motion.span className="h-[6px] w-[6px] rounded-full bg-[#22c55e]"
              animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 1.5, repeat: Infinity }} />
            <span className="text-[13px] font-medium" style={{ color: 'rgba(255,255,255,0.6)' }}>{statusText}</span>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-[13px] font-bold tabular-nums" style={{ color: '#6366f1' }}>{Math.floor(progress)}%</span>
            {!showCancel ? (
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="hidden md:block">
            <div className="rounded-2xl p-5 space-y-4" style={{ background: 'rgba(20,20,35,0.7)', border: '1px solid rgba(100,100,180,0.15)', backdropFilter: 'blur(16px)', boxShadow: '0 4px 24px rgba(0,0,0,0.3), 0 1px 0 rgba(255,255,255,0.04) inset' }}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded tracking-wider"
                    style={{ color: badgeStyle.color, background: badgeStyle.bg, border: `1px solid ${badgeStyle.border}` }}>{fileType}</span>
                </div>
              </div>
              <div style={{ height: '1px', background: 'rgba(255,255,255,0.05)' }} />
              <div className="space-y-2.5">
                {[{ label: 'Size', value: `${(file.size / 1024).toFixed(1)} KB` }, { label: 'Uploaded', value: uploadTime.current }, { label: 'Format', value: fileExt }].map(({ label, value }) => (
              ...
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.12)' }}>
                <motion.span className="h-[6px] w-[6px] rounded-full bg-[#6366f1] shrink-0" animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 1.5, repeat: Infinity }} />
                <span className="text-[11px] font-medium" style={{ color: '#6366f1' }}>{progress >= 100 ? 'Complete' : 'AI Processing'}</span>
              </div>
            </div>
          </motion.div>
            ...
            <div className="rounded-2xl p-4 sm:p-8 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(20,20,35,0.85) 0%, rgba(30,25,55,0.7) 100%)', border: '1px solid rgba(100,100,180,0.15)', boxShadow: '0 25px 60px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.05)', backdropFilter: 'blur(20px)' }}>
              ...
              <div className="flex items-start gap-5 mb-10">
                <div className="relative shrink-0 w-[72px] h-[72px]">
                  ...
                  {/* Continuously rotating arc — linear, no easing, no stutter */}
                  <motion.svg
                  ...
                  >
                    ...
                    <circle cx="36" cy="36" r="30" fill="none"
                      ...
                      strokeLinecap="round"
                      strokeDasharray="80 108"
                      strokeDashoffset="0"
                    />
                  </motion.svg>
                  {/* Inner icon — independent slow pulse */}
                  <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    >
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                        <path d="M12 2L13.09 8.26L18 6L14.74 10.91L21 12L14.74 13.09L18 18L13.09 15.74L12 22L10.91 15.74L6 18L9.26 13.09L3 12L9.26 10.91L6 6L10.91 8.26L12 2Z" fill="#a78bfa" />
                      </svg>
                    </motion.div>
                  </div>
                </div>
              ...
              {/* Step-synced message bar */}
              <AnimatePresence mode="wait">
                <motion.div key={`${activeStepIdx}-${msgIdx}`}
                  ...
                  <span className="text-[13px]" style={{ color: 'rgba(255,255,255,0.45)' }}>{currentMsg}</span>
                </motion.div>
              </AnimatePresence>
                        ...
                        {st === 'done' && (
                          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 400, damping: 15 }}>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                              <circle cx="12" cy="12" r="10" stroke="#22c55e" strokeWidth="1.5" fill="rgba(34,197,94,0.08)" />
            ...
            <div className="rounded-2xl relative overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(20,20,35,0.7) 0%, rgba(25,22,45,0.5) 100%)', border: '1px solid rgba(100,100,180,0.12)', boxShadow: '0 20px 50px rgba(0,0,0,0.4)', backdropFilter: 'blur(16px)' }}>
              <div className="px-5 py-3 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-semibold tracking-[0.15em] uppercase" style={{ color: 'rgba(255,255,255,0.4)' }}>{showScanLine ? 'Analyzing content' : 'Analysis complete'}</span>
                </div>
              ...
              <div className="relative p-5" style={{ minHeight: '460px' }}>
                ...
                {showScanLine && (
                  <motion.div className="absolute left-0 right-0 z-20 pointer-events-none"
                    ...
                    <div style={{ height: '2px', background: 'linear-gradient(90deg, transparent 0%, #6366f1 20%, #a78bfa 50%, #6366f1 80%, transparent 100%)', boxShadow: '0 0 8px rgba(99,102,241,0.8), 0 0 20px rgba(139,92,246,0.4)', filter: 'blur(0.3px)' }} />
                ...
                {showScanLine && (
                  <motion.div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 flex items-center gap-1.5 px-3 py-1.5 rounded-full pointer-events-none"
                    ...
                    <motion.span className="h-[5px] w-[5px] rounded-full bg-[#6366f1]" animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 1, repeat: Infinity }} />
